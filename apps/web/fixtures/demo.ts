 ...
 *    · пиковая ночь «послезавтра» занята полностью, 0 свободных из 10.
 *
 *  Почему пик именно на послезавтра, а не на завтра. Шторка создания брони
 *  открывается с умолчанием «сегодня, 2 ночи», то есть окном [0, +2).
 *  Когда полностью занята была ночь +1, это окно НИКОГДА не имело
 *  свободных номеров, и главный сценарий продукта с порога упирался
 *  в «Свободных номеров нет». Пик сдвинут на +2: окно по умолчанию
 *  находит номер 4, а пиковая ночь по-прежнему видна в «Ночах» и «Сетке».
...
const SEED: Seed[] = [
	["Сыдыков Айбек", "1", -2, 4, "CHECKED_IN", "manual"],
	...
	// Номер 4 освобождается сегодня и занимается только послезавтра —
	// это тот самый номер, который находит шторка создания по умолчанию.
	["Ибраимова Айгүл", "4", -2, 0, "CHECKED_IN", "manual"],
	["Мамбетова Асель", "4", 2, 6, "CONFIRMED", "manual"],
	["Жумабаев Асан", "5", -1, 3, "CHECKED_IN", "manual"],
	["Ли Наталья", "6", 0, 4, "CONFIRMED", "manual"],
	["Familie Weber", "7", -1, 5, "CHECKED_IN", "manual"],
	["Wang Jing", "8", -3, 4, "CHECKED_IN", "manual"],
	["Асанова Гүлнара", "Ю1", 0, 4, "CONFIRMED", "manual"],
