...
| Спринт | Статус |
| --- | --- |
| Sprint 0 — Setup | ✅ код готов (миграцию сгенерировать при первом запуске: `prisma migrate dev`) |
...
## Ограничения сборки в песочнице

Код написан без доступа к npm registry: зависимости не устанавливались, полный запуск
не выполнялся. Проверены юнит-тестами без зависимостей: доменное ядро броней
(`src/bookings/domain.test.ts`) и диалоговая машина бота (`src/telegram/bot-flow.test.ts`).
После переноса: `pnpm install` → `prisma migrate dev` → `pnpm test` → `pnpm test:races`.
